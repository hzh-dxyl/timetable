...
@WebServlet(name = "modifyServlet", urlPatterns = "/servlet.modifyServlet")
public class modifyServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String type = request.getParameter("type");  //"user"为用户自定义。"credit"为学分制
        if (type.equals("user")) {
            ...
            if (dbBean.modifyLesson(lesson, txtUserID))
                response.getWriter().println("success");
            else response.getWriter().println("error");
        } else if (type.equals("credit")) {
            String note = request.getParameter("note");
            if (dbBean.modifyNote(request.getParameter("name"), txtUserID, note))
                response.getWriter().println("success");
            else response.getWriter().println("error");
        }
    }
